...
const categoryType = ref(1) // 红包和商家代金券切换
const hongbaoList = ref(null) // 红包列表

const initData = (user) => {
  if (user) {
    getHongbaoNum(user.user_id).then(res => {
      hongbaoList.value = res;
    ...
    <section v-if="!showLoading">
      ...
      <transition name="router-fade">
        <section v-if="categoryType === 1">
          <section class="hongbao_container">
            <header class="hongbao_title">
              <section class="total_number">
                有<span>{{ hongbaoList.length }}</span
                >个红包即将到期
              </section>
              <section class="hongbao_description">
                <img src="@/assets/description.png" width="24" height="24" alt="" />
                <router-link to="/discount/hbDescription" class="hongbao_detail"
                  >红包说明
                </router-link>
              </section>
            </header>
          ...
          <router-link to="/discount/hbHistory" class="history_hongbao">
          ...
          <footer class="hongbao_footer">
            <router-link
              to="/discount/exchange"
              class="hongbao_style"
              style="border-right: 1px solid #f5f5f5"
            >
              兑换红包
            </router-link>
            <router-link to="/discount/commend" class="hongbao_style">推荐有奖</router-link>
          </footer>
        </section>
      </transition>
      <transition name="router-fade">
        <section v-if="categoryType === 2" class="voucher_container">
          <section class="hongbao_description voucher_header">
            <img src="@/assets/description.png" width="24" height="24" alt="" />
            <router-link to="/discount/coupon" class="hongbao_detail">商家代金券说明</router-link>
          </section>
          <section class="unable_use">
            <img src="../../assets/voucher.png" height="170" width="300" alt="" />
            <p>无法使用代金券</p>
            <p>非客户端或客户端版本过低</p>
