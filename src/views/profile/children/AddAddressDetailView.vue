...
const { addAddress, setState } = store
...
const inputThing = () => {
  ...
  searchNearby(inputAddress.value).then((res) => {
...
const listClick = (index) => {
  setState({ addAddress: addressList.value[index].name })
  $router.go(-1)
}

onBeforeMount(() => {
  inputAddress.value = addAddress ? addAddress : inputAddress.value
  inputThing()
})
</script>
