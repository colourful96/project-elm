...
import FoodGuide from '@/components/footer/FootGuide.vue'
import { imgBaseUrl, getStore, setStore } from '@/utils/index.js'
import { searchRestaurant } from '@/service/getData.js'

const $route = useRoute()

const searchValue = ref('')
const showHistory = ref(true) // 是否显示历史记录，只有在返回搜索结果后隐藏
const restaurantList = ref([]) // 搜索结果
const geohash = ref('')
const searchHistory = ref([]) // 搜索历史记录
const emptyResult = ref(false) // 搜索结果为空

const checkInput = () => {
  if (searchValue.value === '') {
    showHistory.value = true //显示历史记录
    restaurantList.value = [] //清空搜索结果
    emptyResult.value = false //隐藏搜索为空提示
  }
}
const searchTarget = async (historyValue) => {
  if (historyValue) {
    searchValue.value = historyValue
  } else if (!searchValue.value) {
    return
  }
  // 隐藏历史记录
  showHistory.value = false
  restaurantList.value = await searchRestaurant(geohash.value, searchValue.value)
  emptyResult.value = !restaurantList.value.length

  const tempHistory = [...new Set([...searchHistory.value, searchValue.value])]
  searchHistory.value = tempHistory
  setStore('searchHistory', tempHistory)
}
// 清空搜索历史
const clearAllHistory = () => {
  searchHistory.value = []
  setStore('searchHistory', searchHistory.value)
}
...
onMounted(() => {
  ...
  if (getStore('searchHistory')) {
    searchHistory.value = JSON.parse(getStore('searchHistory'))
  }
})
</script>
            ...
            <div class="item_right_text">
              <p>
                ...
                >
                  ...
                  />
                  <text x="3.5" y="9" style="fill: #ff6000; font-size: 9; font-weight: bold">
                    支付
                  </text>
                </svg>
              </p>
