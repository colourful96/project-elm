...
const initData = async () => {
  const unique_id = $route.params.order_id;
  if (userInfo.value && userInfo.value.user_id && unique_id) {
    orderData.value = await getOrderDetail(userInfo.value.user_id, unique_id);
    ...
    <section v-if="!showLoading">
      <section class="scroll_insert">
        <section class="order_titel">
          <img :src="imgBaseUrl + orderData.restaurant_image_url" alt=""/>
          <p>{{ orderData.status_bar.title }}</p>
          <p>{{ orderData.timeline_node.description }}</p>
          <router-link
            class="order_again"
            :to="{ path: '/shop', query: { geohash, id: orderData.restaurant_id } }"
          >
            再来一单
          </router-link>
        </section>
        <section class="food_list">
          ...
          <div class="deliver_fee">
            <span>配送费</span>
            <span>{{
              (orderData.basket.deliver_fee && orderData.basket.deliver_fee.price) || 0
            }}</span>
          </div>
