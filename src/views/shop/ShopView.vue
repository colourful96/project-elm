...
import { setStore, imgBaseUrl as baseUrl, getImgPath } from '@/utils/index.js'
import {
  msiteAddress,
  shopDetails,
  foodMenu,
...
const showLoading = ref(true) // 加载状态
const showActivities = ref(false) // 是否显示活动详情
const imgBaseUrl = ref(baseUrl)
const geohash = ref('')
const shopId = ref(null)
const shopDetailData = ref(null) // 商铺详情信息
const changeShowType = ref('food') // 显示食品列表还是评价列表
const menuList = ref([])
const TitleDetailIndex = ref(null) // 食品列表头部详情
const categoryNum = ref([]) // 已加入购物车数量
const menuIndex = ref(0) // 选中的菜单索引值
const totalPrice = ref(0) // 总价格
const receiveInCart = ref(false) // 购物车组件下落的圆点是否到达目标位置
const showSpace = ref(false) // 食品规格显隐
const spaceIndex = ref(0) //当前选中的规格索引值
const choosedFoods = ref(null) //当前选中食品数据
const showMoveDot = ref([])
const elLeft = ref(0) // 当前点击加按钮在网页中的绝对left值
const elBottom = ref(0) // 当前点击加按钮在网页中的绝对top值
const windowHeight = ref(null) // 屏幕的高度
const showDeleteTip = ref(false) //多规格商品点击减按钮，弹出提示框
const showCartList = ref(false) // 显示购物车列表
const cartFoodList = ref([]) // 购物车商品列表
const preventRepeatRequest = ref(false) // 是否正在请求数据中
const loadRatings = ref(false) //加载更多评论是显示加载组件
const ratingOffset = ref(0) //评价获取数据offset值
const ratingList = ref(null) // 评论列表
const ratingScoresData = ref(null) // 商铺总体评论
const ratingTagsList = ref(null) // 评论分类列表
const ratingTageIndex = ref(0) // 评价分类索引
const ratingTagName = ref('') // 评论的类型
const ratingScroll = ref(null) // 评论页的scroll
const cartContainer = ref(null)

let timer = null

const promotionInfo = computed(() => {
  return shopDetailData.value.promotion_info || '欢迎光临，用餐高峰期请提前下单，谢谢。'
})
// 购物车中商品总数量
...
const minimumOrderAmount = computed(() => {
  if (shopDetailData.value) {
    return shopDetailData.value.float_minimum_order_amount - totalPrice.value
  } else {
    return null
  }
})
...
const shopCart = computed(() => {
  return { ...store.cartList[shopId.value] }
})
// 配送费
...
const initData = async () => {
  if (!store.latitude) {
    const res = await msiteAddress(geohash.value)
    store.setState({
  ...
  // 获取商铺详情
  shopDetailData.value = await shopDetails(shopId.value, store.latitude, store.longitude)
  // 获取的是该商铺的食品分类
  menuList.value = await foodMenu(shopId.value)
  menuList.value = menuList.value.slice(0, 3).map((menu) => {
    return {
      ...menu,
  ...
  // 评论列表
  ratingList.value = await getRatingList(shopId.value, ratingOffset.value)
  // 商铺总体评价
  ratingScoresData.value = await ratingScores(shopId.value)
  // 获取评论分类列表
  ratingTagsList.value = await ratingTags(shopId.value)
  // 记录商铺详情
  store.record_shopDetail(shopDetailData.value)
...
const toggleCartList = () => {
  ...
  console.log(showCartList.value, 'this.showCartList')
  console.log(cartFoodList.value, 'this.cartFoodList')
}
// 显示规格列表
const showChooseList = (foods) => {
  if (foods) {
    this.choosedFoods.value = foods
  }
...
const showMoveDotFun = (showMoveDot, elLeft, elBottom) => {
  showMoveDot.value = [...showMoveDot.value, ...showMoveDot.value]
  elLeft.value = elLeft
  elBottom.value = elBottom
}
const beforeEnter = (el) => {
  el.style.transform = `translate3d(0, ${37 + elBottom.value - windowHeight.value}px,0)`
  el.children[0].style.transform = `translate3d(${elLeft.value - 30}px,0,0)`
...
const clearCart = () => {
  toggleCartList()
  clear_cart(shopId.value)
}
// 移出购物车
const removeOutCart = (category_id, item_id, food_id, name, price, specs) => {
  store.reduce_cart({ shopid: shopId.value, category_id, item_id, food_id, name, price, specs })
}
// 加载更多评论
const loaderMoreRating = async () => {
  if (preventRepeatRequest.value) return
  loadRatings.value = true
  preventRepeatRequest.value = true
  ratingOffset.value += 10
  let ratingDate = await getRatingList(shopId.value, ratingOffset.value, ratingTagName.value)
  ratingList.value = [...ratingList.value, ...ratingDate]
...
// 获取不同类型的评论列表
const changeTagIndex = async (index, name) => {
  ratingTageIndex.value = index
  ratingOffset.value = 0
  ratingTagName.value = name
  let res = await getRatingList(this.shopId.value, this.ratingOffset.value, name)
...
const jumpFoodDetail = (foods) => {
  setStore('foodDetail', foods)
  $router.push('/shop/foodDetail')
}
...
watch(() => changeShowType.value, (value) => {
  if (value === 'rating') {
    nextTick(() => {
      ...
      ratingScroll.value.on('scroll', (pos) => {
        if (Math.abs(Math.round(pos.y)) >= Math.abs(Math.round(ratingScroll.value.maxScrollY))) {
          loaderMoreRating();
          ratingScroll.value.refresh();
        }
      })
...
<template>
  <div>
        ...
        <section class="description_header">
          <router-link :to="{
            ...
            <section class="description_right">
              ...
              <p class="description_promotion ellipsis">{{ promotionInfo }}</p>
            </section>
            <svg
              width="14"
              height="14"
      ...
      <section class="change_show_type" ref="chooseType">
        <div>
          <span
            ...
            @click.stop="changeShowType = 'food'"
            >商品</span
          >
        </div>
        <div>
          <span
            ...
            @click.stop="changeShowType = 'rating'"
            >评价</span
          >
        </div>
      </section>
              ...
              <ul>
                <li v-for="(item, index) in menuList" :key="index">
                  <header class="menu_detail_header">
                    ...
                    <span class="menu_detail_header_right" @click.stop="showTitleDetail(index)"></span>
                  ...
                  >
                    <div @click.stop="jumpFoodDetail(foods)" class="menu_detail_link">
          ...
          <section class="buy_cart_container">
            ...
            <section class="gotopay" :class="{ gotopay_acitvity: minimumOrderAmount <= 0 }">
              <span class="gotopay_button_style" v-if="minimumOrderAmount > 0"
                >还差¥{{ minimumOrderAmount }}元起送</span
              >
              <router-link
            ...
            <section>
              ...
              <ul class="tag_list_ul">
                <li
                  ...
                  :key="index"
                  @click.stop="changeTagIndex(index, item.name)"
    ...
    <section>
      <transition name="fade">
        <div class="specs_cover" @click.stop="showChooseList" v-if="showSpace"></div>
      </transition>
          ...
          <section class="specs_details">
            ...
            <ul>
              <li
                ...
                :class="{ specs_activity: itemIndex == spaceIndex }"
            ...
            <div
              ...
              @click.stop="
                addSpace(
                  ...
                  choosedFoods.specfoods[spaceIndex].food_id,
                  choosedFoods.specfoods[spaceIndex].name,
                  choosedFoods.specfoods[spaceIndex].price,
                  choosedFoods.specifications[0].values[spaceIndex],
                  choosedFoods.specfoods[spaceIndex].packing_fee,
                  choosedFoods.specfoods[spaceIndex].sku_id,
                  choosedFoods.specfoods[spaceIndex].stock,
                )
              "
            >
              加入购物车
