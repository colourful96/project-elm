...
import { shopDetails } from '@/service/getData.js'

const store = useStore()
const $route = useRoute()

const shopDetail = computed(() => store.shopDetail || {})

const licenseImg = ref(null)
const showlicenseImg = ref(false)

const showLicenseImg = (img) => {
  licenseImg.value = img
  showlicenseImg.value = true
}
onMounted(async () => {
  const shopDetailData = await shopDetails($route.query.shopId, $route.query.latitude, $route.query.longitude)
  store.record_shopDetail(shopDetailData)
})
</script>
...
<template>
  <div class="rating_page">
      ...
      <section class="shop_statu_detail">
        ...
        <div class="check_date">
          <p>
            ...
            <span class="shop_status_bad" v-else>差</span>w
          </p>
          <p>
            <span>检查日期：</span>
            <span>{{
    ...
    <section class="shop_status_info">
      ...
      <p>营业时间：[{{ shopDetail.opening_hours ?  shopDetail.opening_hours[0] : '' }}]</p>
    ...
    <transition name="fade">
      <section class="license_container" v-if="showlicenseImg" @click="showlicenseImg = false">
        <img :src="imgBaseUrl + licenseImg" alt="" />
      </section>
    </transition>
  </div>
</template>
